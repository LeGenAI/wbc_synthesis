...
The real-only LODO baseline (B0) yields a mean macro-F1 of
%%BASELINE_MACRO_F1_AVG%% across four held-out domains, confirming that
the benchmark is meaningfully challenging. Easy validation-split
evaluations, by contrast, exceed 0.90 macro-F1 and mask any synthetic
utility signal.
...
\begin{itemize}[leftmargin=*,nosep]
\item \textbf{Monocyte rescue on Raabin:} Baseline F1 of
  %%HARDCLASS_MONOCYTE_F1_RAABIN%% improves with class-specific policy.
\item \textbf{Eosinophil rescue on AMC:} Baseline F1 of
  %%HARDCLASS_EOSINOPHIL_F1_AMC%% improves with hard-class-focused
  generation, providing the largest macro-F1 gain.
