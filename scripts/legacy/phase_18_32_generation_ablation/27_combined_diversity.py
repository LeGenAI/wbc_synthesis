"""
Script 27: 다중 입력 × IP-Adapter 스타일 결합 실험
====================================================
Script 25 (다중 입력 5장)와 Script 23 (IP-Adapter 스타일 참조)를 결합.

핵심 아이디어:
  - 입력 다양성 (Script 25): random.sample 5장 → Inter-input SSIM ~0.46
  - 스타일 다양성 (Script 23): IP-Adapter 참조 이미지 → 배경/염색 스타일 주입
  - 결합 (Script 27): 두 가지를 동시에 → 최대 다양성

비교 조건 (같은 입력 이미지로 3가지 조건 비교):
  A: ip_scale=0.0, n_ref=0  (기준선, Script 25 재현)
  B: ip_scale=0.2, n_ref=1  (IP-Adapter 약)
  C: ip_scale=0.35, n_ref=2 (IP-Adapter 강)

총 생성: 5cls × 4dom × 5입력 × 3seed × 3조건 = 900장
예상 시간: 파이프라인+IP-Adapter 로드 ~7분 + 900장 × ~5초 = ~82분

Usage:
    python3 scripts/legacy/phase_18_32_generation_ablation/27_combined_diversity.py
    python3 scripts/legacy/phase_18_32_generation_ablation/27_combined_diversity.py --n_inputs 3 --n_seeds 2
"""
...
OUT_DIR     = ROOT / "results" / "combined_diversity"
...
CONDITIONS = {
    ...
    "B": {"ip_scale": 0.2,  "n_ref": 1, "label": "IP-Adapter 약 (ip=0.2, ref=1)"},
    "C": {"ip_scale": 0.35, "n_ref": 2, "label": "IP-Adapter 강 (ip=0.35, ref=2)"},
}
...
def make_gallery(all_results: list, n_inputs: int, n_seeds: int) -> str:
    ...
    lines += [
        "# WBC Combined Diversity 갤러리 (입력 다양성 × IP-Adapter 스타일)",
        "",
        f"> **Strength:** {STRENGTH} 고정  ",
        f"> **N_INPUTS:** {n_inputs}  ·  **N_SEEDS:** {n_seeds}  ",
        f"> **총 생성:** {5 * 4 * n_inputs * n_seeds * 3}장 (3조건 × 입력 × seed)  ",
        "",
        "## 실험 조건",
        "",
    ...
    for cls in CLASSES:
        ...
        for dom in DOMAINS:
            ...
            for inp_data in entry["inputs"]:
                ...
                ref_b_rel = f"images/{cls}/{dom}/input_{inp_idx:02d}/ref_B.png"
                ref_c0_rel = f"images/{cls}/{dom}/input_{inp_idx:02d}/ref_C_00.png"
                ref_c1_rel = f"images/{cls}/{dom}/input_{inp_idx:02d}/ref_C_01.png"

                lines.append(f"### 입력 {inp_idx}")
                lines.append("")

                # 입력 + 참조 이미지
                lines.append("| 원본 입력 | 참조 B (1장) | 참조 C-0 | 참조 C-1 |")
                lines.append("| :---: | :---: | :---: | :---: |")
                lines.append(
                    f"| ![inp{inp_idx}]({inp_rel}) "
                    f"| ![refB]({ref_b_rel}) "
                    f"| ![refC0]({ref_c0_rel}) "
                    f"| ![refC1]({ref_c1_rel}) |"
                )
    ...
    lines.append("> - B/C 조건의 Inter SSIM < A 조건이면 IP-Adapter가 다양성 증가에 기여한 것  ")
...
def main():
    ...
    print("=" * 65)
    print("Script 27: 다중 입력 × IP-Adapter 스타일 결합 실험")
    ...
    ref_pool   = {}   # (cls, dom_key, inp_idx) → {"B": [img], "C": [img, img]}
        ...
        for dom in DOMAINS:
            ...
            for inp_idx in range(len(inputs)):
                ref_pool[(cls, dom_key, inp_idx)] = {
                    "B": sampler.sample(cls, exclude_domain=dom_key, n=1),
                    "C": sampler.sample(cls, exclude_domain=dom_key, n=2),
                }
    ...
    for cls in CLASSES:
        for dom in DOMAINS:
            ...
            for inp_idx, inp_path in enumerate(inputs):
                ...
                refs = ref_pool.get((cls, dom_key, inp_idx), {"B": [], "C": []})

                # 참조 이미지 저장
                if refs["B"]:
                    refs["B"][0].save(inp_dir / "ref_B.png")
                for ri, rimg in enumerate(refs["C"]):
                    rimg.save(inp_dir / f"ref_C_{ri:02d}.png")
                    ...
                    for k in active_conds:
                        ...
                        if save_path.exists():
                            ...
                            if n_ref == 0:
                                ...
                                ref_imgs = refs["B"][:n_ref]
                            else:  # C
                                ref_imgs = refs["C"][:n_ref]
