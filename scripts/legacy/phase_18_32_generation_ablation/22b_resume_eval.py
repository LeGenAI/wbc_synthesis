"""
Script 22b: Script 22 Resume — monocyte(Raabin~AMC) + neutrophil 재실행
==========================================================================
이미 생성된 이미지(gen_*.png가 N_SEEDS개 존재)는 건너뛰고 CNN 평가만 수행.
미생성 조합만 img2img 생성 후 평가.
최종적으로 300장 전체 summary.json + report.md 저장.
"""
...
def main():
    ...
    print("=" * 65)
    print("Script 22b: 5클래스 × 4도메인 × Strength 전체 매트릭스 (Resume)")
    print(f"  Strengths: {STRENGTHS}")
    print(f"  Seeds/strength: {N_SEEDS}")
    print(f"  총 생성: {n_total}장 (기존 생성분 skip)")
    print("=" * 65)
    ...
    for cls_name in MULTI_CLASSES:
        for dom in DOMAINS:
            ...
            # rng 상태는 원본과 동일하게 유지 (같은 파일 선택 보장)
            ...
            # ── 이 조합이 이미 완전히 완료됐는지 확인 ─────────────────
            existing_total = len(list(out_dir.glob("s*/gen_*.png")))
            all_done = existing_total >= N_SEEDS * len(STRENGTHS)

            if all_done:
                print(f"\n  ✅ skip  {cls_name} × {DOMAIN_SHORT[dom]} (이미 {existing_total}장 존재)")
                # 기존 이미지로 matrix 복원 (CNN 재평가)
                per_strength = []
                ref_img_path = out_dir / "input.png"
                ref_img = Image.open(ref_img_path).convert("RGB") if ref_img_path.exists() else None

                for strength in STRENGTHS:
                    s_dir = out_dir / f"s{int(strength*100):03d}"
                    gen_paths = sorted(s_dir.glob("gen_*.png"))[:N_SEEDS]
                    gen_imgs = [Image.open(p).convert("RGB") for p in gen_paths]

                    ssims = [round(ssim_pair(g, ref_img), 4) for g in gen_imgs] if ref_img else [0.0]*N_SEEDS
                    ...
                    print(f"    s={strength}  SSIM={np.mean(ssims):.4f}  "
            ...
            # input.png 저장 (없는 경우만)
            input_png = out_dir / "input.png"
            if not input_png.exists():
                input_png.write_bytes(chosen.read_bytes())
            ...
            for strength in STRENGTHS:
                ...
                # ── strength 단위 skip ────────────────────────────────
                existing_seeds = sorted(s_dir.glob("gen_*.png"))
                if len(existing_seeds) >= N_SEEDS:
                    # 기존 파일 로드 후 CNN 평가
                    gen_imgs = [Image.open(p).convert("RGB") for p in existing_seeds[:N_SEEDS]]
                    ...
                    print(f"    s={strength} [skip] SSIM={np.mean(ssims):.4f}  "
                          f"CNN={n_correct}/{len(preds)} ({cnn_acc*100:.0f}%)  {pred_dist}")
                ...
                # ── 실제 생성 ─────────────────────────────────────────
                gen_imgs = []
                # 이미 일부 생성된 seed가 있을 수 있음
                start_seed = len(existing_seeds)
                # 기존 partial 이미지 로드
                for p in existing_seeds:
                    gen_imgs.append(Image.open(p).convert("RGB"))
                ...
                for seed in range(start_seed, N_SEEDS):
                    ...
                    total_done += 1
                print(f" [{total_done + total_skipped}/{n_total}]")
    ...
    for cls_name in MULTI_CLASSES:
        ...
        for s_idx, strength in enumerate(STRENGTHS):
            accs = [matrix[cls_name][dom][s_idx]["cnn_acc"]
                    for dom in DOMAINS if dom in matrix[cls_name] and len(matrix[cls_name][dom]) > s_idx]
            if accs:
    ...
    for dom in DOMAINS:
        ...
        for s_idx, strength in enumerate(STRENGTHS):
            accs = [matrix[cls][dom][s_idx]["cnn_acc"]
                    ...
                    if dom in matrix[cls] and len(matrix[cls][dom]) > s_idx]
            if accs:
                vals.append(f"{np.mean(accs)*100:5.0f}%")
            else:
                vals.append("  N/A")
    ...
    print(f"\n  신규 생성: {total_done}장 / Skip: {total_skipped}장")
    print(f"\n✅ 완료! → {OUT_DIR}")
