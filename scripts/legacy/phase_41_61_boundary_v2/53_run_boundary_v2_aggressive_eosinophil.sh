...
if [[ -d "$ROOT/results/boundary_v2_generation/eosinophil" ]]; then
  cp -R "$ROOT/results/boundary_v2_generation/eosinophil" \
    "$ARCHIVE_ROOT/eosinophil_probe_bg065_rf020_n2_$STAMP"
fi

echo "[aggressive probe] eosinophil"
"$PYTHON" "$ROOT/scripts/legacy/phase_41_61_boundary_v2/45_background_aware_generate.py" \
  ...
  --disable_refine \
  --force \
  2>&1 | tee "$LOG_DIR/eosinophil_v2_aggressive_$STAMP.log"

echo "[done] eosinophil aggressive boundary V2 probe"
