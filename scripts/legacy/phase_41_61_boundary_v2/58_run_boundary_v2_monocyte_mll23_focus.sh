...
if [[ -d "$ROOT/results/boundary_v2_generation/monocyte" ]]; then
  cp -R "$ROOT/results/boundary_v2_generation/monocyte" \
    "$ARCHIVE_ROOT/monocyte_probe_bg095_mll23_focus_n${N_PER_DOMAIN}_s${N_SEEDS}_$STAMP"
fi

echo "[1/2] monocyte bg095 focused harvest (MLL23 -> PBC/Raabin)"
"$PYTHON" "$ROOT/scripts/legacy/phase_41_61_boundary_v2/45_background_aware_generate.py" \
  ...
  --target_domains domain_a_pbc domain_b_raabin \
  --force \
  2>&1 | tee "$LOG_DIR/monocyte_v2_bg095_mll23_focus_n${N_PER_DOMAIN}_s${N_SEEDS}_$STAMP.log"
...
"$PYTHON" "$ROOT/scripts/legacy/phase_41_61_boundary_v2/46_boundary_subset_builder.py" \
  ...
  2>&1 | tee "$LOG_DIR/boundary_subset_builder_mll23_focus_mm${MARGIN_MAX/./}_bg${BG_SSIM_MAX/./}_$STAMP.log"

echo "[done] monocyte bg095 MLL23-focused harvest finished"
