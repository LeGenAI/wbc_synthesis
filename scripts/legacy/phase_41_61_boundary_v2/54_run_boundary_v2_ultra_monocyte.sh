...
if [[ -d "$ROOT/results/boundary_v2_generation/monocyte" ]]; then
  cp -R "$ROOT/results/boundary_v2_generation/monocyte" \
    "$ARCHIVE_ROOT/monocyte_probe_bg075_bg085_rf000_n1_$STAMP"
fi

echo "[ultra probe] monocyte"
"$PYTHON" "$ROOT/scripts/legacy/phase_41_61_boundary_v2/45_background_aware_generate.py" \
  ...
  --disable_refine \
  --force \
  2>&1 | tee "$LOG_DIR/monocyte_v2_ultra_$STAMP.log"

echo "[done] monocyte ultra boundary V2 probe"
