...
def log_validation(
    ...
    if args.num_validation_images <= 0:
        logger.info("Skipping validation because num_validation_images <= 0")
        return []
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        metadata_jsonl = None
        if instance_data_root is not None:
            metadata_jsonl = Path(instance_data_root) / "metadata.jsonl"

        if args.dataset_name is not None or (metadata_jsonl is not None and metadata_jsonl.exists()):
            try:
            ...
            if args.dataset_name is not None:
                ...
                dataset = load_dataset(
                    "imagefolder",
                    data_dir=str(instance_data_root),
                    cache_dir=args.cache_dir,
                )
